...
const form = document.querySelector<HTMLFormElement>(".lorem-form");
const amount = document.getElementById("amount") as HTMLInputElement;
const result = document.querySelector<HTMLElement>(".lorem-text");
...
form?.addEventListener("submit", (e) => {
  ...
  const value: number = parseInt(amount?.value);
  const random: number = Math.floor(Math.random() * text.length);

  if (isNaN(value) || value < 0 || value > 9) {
    result!.innerHTML = `<p class="result">${text[random]}</p>`;
  } else {
    let tempText = text.slice(0, value);
    let chosenText = tempText
      .map(function (item) {
        return `<p class="result">${item}</p>`;
      })
      .join("");
    result!.innerHTML = chosenText;
  }
});
